...
namespace AnimeFightClub001
{
    ...
    {
        #region Declarations
        protected float m_masterVol; //Stores the master volume
        protected float m_gameVol; //Stores the game volume
        protected float m_musicVol; //Stores the music volume
        ...
        public void ReadFromConfig()
        {
            ...
            foreach (KeyValuePair<string, string> pair in configDict)
            {
                ...
                else if (pair.Key == "Master Volume")
                {
                    int tempVolume = Convert.ToInt32(pair.Value);
                    if (tempVolume < 0)
                    {
                        tempVolume = 0;
                    }
                    ...
                    m_masterVol = (float)tempVolume / 100;
                }
                else if (pair.Key == "Game Volume")
                {
                    int tempVolume = Convert.ToInt32(pair.Value);
                    if (tempVolume < 0)
                    {
                    ...
                    m_gameVol = (float)tempVolume / 100;
                }
                else if (pair.Key == "Music Volume")
                {
                    int tempVolume = Convert.ToInt32(pair.Value);
                    if (tempVolume < 0)
                    {
                    ...
                    m_musicVol = (float)tempVolume / 100;
                }
            }
        }
