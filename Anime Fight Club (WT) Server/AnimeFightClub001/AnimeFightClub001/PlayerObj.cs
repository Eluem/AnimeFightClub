...
namespace AnimeFightClub001
{
    ...
    class PlayerObj : BasicObj
    {
        ...
        protected int m_regenCounter; //Used for mana regeneration.
        protected int m_grabCounter;
        protected int m_hp;
        protected int m_mp;
        protected int m_attack;
        protected int m_defense;
        protected bool m_canDoubleJump; //Self-explanatory.
        protected bool m_oldJump; //Distinguishes a new press of the jump button.
        ...
        protected int m_kills; //Stores the number of kills a player 
        ...
        const float NORM_PHYS_WIDTH = 40; //Normal width of the player's collision box
        const float NORM_PHYS_HEIGHT = 100; //Normal height of the player's collision box

        const float SLIDING_PHYS_WIDTH = 100; //Sliding width of the player's collision box
        const float SLIDING_PHYS_HEIGHT = 60; //Sliding height of the player's collision box
        #endregion
        ...
        public override void NetSendMe(NetOutgoingMessage sendMsg)
        {
            ...
            sendMsg.Write(m_userName);

            sendMsg.Write(HP);
            sendMsg.Write(MP);

            sendMsg.Write(m_ledgeGrabPos.X);
            sendMsg.Write(m_ledgeGrabPos.Y);

            m_loadout.NetSendMe(sendMsg);

            m_statusHandler.NetSendMe(sendMsg);
        }
        ...
        //****************************************************
        public PlayerObj(Int64 playerID, string userName, NetIncomingMessage msg)
            : base()
        {
            m_playerID = playerID;
            m_userName = userName;
            ...
            SetPos(GameObjectHandler.GetRespawnPoint());
        }
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            if (!m_statusHandler.HasStatus(StatusName.Death))
            {
                ...
                m_drawableCollisionBox.Draw(spriteBatch);
        ...
        {
            int tempState = DerpLolCludge(state, action);

            NetworkingHandler.SendStateUpdate(this, "PlayerObj", tempState);
        ...
        //****************************************************
        // Method: DerpLolCludge
        //
        // Purpose: Does what ObtainPlayerState on the client
        // so that the server can use the same function name
        // all over the place as the client but can intercept
        // the state so it can be synchronized.
        //****************************************************
        public int DerpLolCludge(PlayerState state, PlayerAction action)
        {
        ...
        public int HP
        {
            ...
            {
                m_hp = value;
                NetworkingHandler.SendPlayerHPUpdate(m_objectID, m_hp);
            }
        }
        public int MP
        {
            ...
            {
                m_mp = value;
                NetworkingHandler.SendPlayerMPUpdate(m_objectID, m_mp);
            }
        }
