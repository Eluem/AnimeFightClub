...
namespace AnimeFightClub001
{
    ...
    class BasicObj
    {
        ...
        protected DrawableObj m_drawableCollisionBox; //Drawable image for the collision box, useful for debugging
        #endregion
        ...
        {
            //Send my type
            sendMsg.Write(this.GetType().ToString().Split('.')[1]);

            //Send ID
            sendMsg.Write(m_objectID);

            //Send owner ID
            sendMsg.Write(m_ownerID);

            //Send owner type
            sendMsg.Write(m_ownerType);

            //Send position
            sendMsg.Write(m_physicsObj.PosX);
            sendMsg.Write(m_physicsObj.PosY);

            //Send velocity
            sendMsg.Write(m_physicsObj.VelX);
            sendMsg.Write(m_physicsObj.VelY);

            //Send Direction
            sendMsg.Write((byte)m_direction);
        ...
        public BasicObj()
        {
            ...
            m_drawableObj = new DrawableObj(this);
            m_drawableObj.Color = Color.Black;
            m_drawableObj.Layer = GlobalVariables.GetLayer(9);

            m_drawableCollisionBox = new DrawableObj("Default", Color.Red, this);
            m_drawableCollisionBox.SetDimensions(m_physicsObj.Rect.Width, m_physicsObj.Rect.Height);
        ...
        public BasicObj(UInt16 ownerID = 0, string ownerType = "World", int despawnClock = -1)
        {
            ...
            m_drawableObj = new DrawableObj(this);
            m_drawableObj.Color = Color.Black;
            m_drawableObj.Layer = .9f; //GlobalVariables.GetLayer(9);

            m_drawableCollisionBox = new DrawableObj("Default", Color.Red, this);
            m_drawableCollisionBox.SetDimensions(m_physicsObj.Rect.Width, m_physicsObj.Rect.Height);
            m_drawableCollisionBox.Layer = .1f;// GlobalVariables.GetLayer(1);
        ...
        {
            m_drawableCollisionBox.Update(gameTime);
            m_drawableCollisionBox.SetDimensions(m_physicsObj.Rect.Width, m_physicsObj.Rect.Height);
        }
        ...
        public virtual void SetDimensions(float Width, float Height)
        {
            ...
            m_drawableObj.SetDimensions(Width, Height);
        }
